...
def Create_company():
# phonenumber,email,addres  name shoud not repete
...
#
# c1=Phone.select().join(Company).where(Company.Company_Name=='Mammoet').get()
# print(c1)
# # # print(c1.select().where(Company.Company_Name=='Mammoet'))
#
# # c1=Phone.select().join(Company).where(Company.Company_Name=='Freight Care Logistics LLC').get()
# # print(c1)
# c1=Phone.select().join(Company).where(Company.Company_Name=='Mammoet').get()
# if Phone.select().where(Phone.Phone_number == Phone_num).exists() | pd.isna(Phone_num):
#     print("exist")
# else:
#     text1 = Phone_num.split('/')
#     # if isinstance(text1, list):
#     print("list", text1)
#     phone = Phone.create(Phone_number=Phone_num, Phone_owner=company)
# Phone_num=data.iloc[i]['Phone Number']
# if Phone.select().where(Phone.Phone_number==Phone_num).exists() | pd.isna(Phone_num):
#     pass
# else:
#     phone = Phone.create(Phone_number=Phone_num, Phone_owner=company)
